...
class TestJointGoals:
    ...
    def test_mimic_joints(self, default_pose_giskard: HSRTester):
        arm_lift_joint = default_pose_giskard.world.search_for_joint_name(
            "arm_lift_joint"
        )
        default_pose_giskard.open_gripper()
        hand_T_finger_current = default_pose_giskard.world.compute_fk(
        ...
        default_pose_giskard.set_joint_goal(js, check=False)
        default_pose_giskard.allow_all_collisions()
        default_pose_giskard.execute()
        np.testing.assert_almost_equal(
            default_pose_giskard.world.state[arm_lift_joint].position, 0.2, decimal=2
        )
        ...
        base_T_torso2 = default_pose_giskard.world.compute_fk(
    ...
    def test_mimic_joints2(self, default_pose_giskard: HSRTester):
        ...
        p.pose.orientation.w = 1
        default_pose_giskard.set_cart_goal(
    ...
    def test_mimic_joints3(self, default_pose_giskard: HSRTester):
        ...
        default_pose_giskard.execute()
        np.testing.assert_almost_equal(
            default_pose_giskard.world.state[arm_lift_joint].position, 0.3, decimal=2
        )
        base_T_torso = PoseStamped()
        base_T_torso.header.frame_id = "base_footprint"
        base_T_torso.pose.position.x = 0
    ...
    def test_mimic_joints4(self, default_pose_giskard: HSRTester):
        ll, ul = default_pose_giskard.world.get_joint_velocity_limits(
            "hsrb/arm_lift_joint"
        )
        assert ll == -0.2
        assert ul == 0.2
        ll, ul = default_pose_giskard.world.get_joint_velocity_limits(
            "hsrb/torso_lift_joint"
        )
        assert ll == -0.1
        assert ul == 0.1
        joint_goal = {"torso_lift_joint": 0.25}
        default_pose_giskard.set_joint_goal(joint_goal, check=False)
        default_pose_giskard.allow_all_collisions()
        default_pose_giskard.execute()
        np.testing.assert_almost_equal(
            default_pose_giskard.world.state["hsrb/arm_lift_joint"].position,
            0.5,
            decimal=2,
        )
...
class TestCartGoals:
    ...
    def test_move_base(self, default_pose_giskard: HSRTester):
        ...
        map_T_odom.pose.orientation = Quaternion(
            *quaternion_about_axis(np.pi / 3, [0, 0, 1])
        )
        ...
        default_pose_giskard.set_cart_goal(base_goal, "base_footprint")
        default_pose_giskard.allow_all_collisions()
        default_pose_giskard.execute()
    ...
    def test_rotate_gripper(self, default_pose_giskard: HSRTester):
        ...
        default_pose_giskard.set_cart_goal(r_goal, default_pose_giskard.tip)
        default_pose_giskard.allow_all_collisions()
        default_pose_giskard.execute()
...
class TestConstraints:
    ...
    def test_open_fridge(self, kitchen_setup: HSRTester):
        ...
        kitchen_setup.set_align_planes_goal(
            tip_link=kitchen_setup.tip, tip_normal=x_gripper, goal_normal=x_goal
        )
        ...
        kitchen_setup.set_open_container_goal(
            tip_link=kitchen_setup.tip, environment_link=handle_name, goal_joint_state=0
        )
        kitchen_setup.allow_all_collisions()
        # kitchen_setup.set_json_goal('AvoidJointLimits', percentage=40)
        kitchen_setup.execute()
...
class TestCollisionAvoidanceGoals:
    ...
    def test_self_collision_avoidance_empty(self, default_pose_giskard: HSRTester):
        default_pose_giskard.execute()
        current_state = default_pose_giskard.world.state.to_position_dict()
    ...
    def test_self_collision_avoidance2(self, default_pose_giskard: HSRTester):
        ...
        default_pose_giskard.set_seed_configuration(js)
        default_pose_giskard.allow_all_collisions()
        default_pose_giskard.execute()
        ...
        default_pose_giskard.set_cart_goal(goal_pose, default_pose_giskard.tip)
        default_pose_giskard.execute()
    ...
    def test_attached_collision1(self, box_setup: HSRTester):
        ...
        box_pose.pose.position = Point(0.85, 0.3, 0.66)
    ...
    def test_collision_avoidance(self, default_pose_giskard: HSRTester):
        ...
        default_pose_giskard.set_joint_goal(js)
        default_pose_giskard.execute()
...
class TestAddObject:
    def test_add(self, default_pose_giskard):
        ...
        default_pose_giskard.set_joint_goal({"arm_flex_joint": -0.7})
        default_pose_giskard.execute()
