...
class CarryMyBullshit(Goal):
    ...
    pub: Publisher = None
    laser_sub: Subscription = None
    point_cloud_laser_sub: Subscription = None
    target_sub: Subscription = None
    ...
    def __init__(
        ...
        if CarryMyBullshit.pub is None:
            CarryMyBullshit.pub = rospy.node.create_publisher(
        ...
        self.max_temp_distance = int(
            self.traj_tracking_radius / self.interpolation_step_size
        )
        ...
        if (
            CarryMyBullshit.point_cloud_laser_sub is None
            and self.point_cloud_laser_topic_name is not None
        ):
            CarryMyBullshit.point_cloud_laser_sub = rospy.node.create_subscription(
                LaserScan,
        ...
        if not self.drive_back:
            if CarryMyBullshit.target_sub is None:
                CarryMyBullshit.target_sub = rospy.node.create_subscription(
                    PointStamped, patrick_topic_name, self.target_cb, 10
                )
            sleep(0.5)
            for i in range(int(wait_for_patrick_timeout)):
                if CarryMyBullshit.trajectory.shape[0] > 5:
                    break
                print(
                    f"waiting for at least 5 traj points, current length {len(CarryMyBullshit.trajectory)}"
                )
                sleep(1)
            else:
        ...
        min_left_violation1 = symbol_manager.get_symbol(
            self.ref_str + ".closest_laser_left"
        )
        min_right_violation1 = symbol_manager.get_symbol(
            self.ref_str + ".closest_laser_right"
        )
        closest_laser_reading1 = symbol_manager.get_symbol(
            self.ref_str + ".closest_laser_reading"
        )
        min_left_violation2 = symbol_manager.get_symbol(
            self.ref_str + ".closest_laser_left_pc"
        )
        min_right_violation2 = symbol_manager.get_symbol(
            self.ref_str + ".closest_laser_right_pc"
        )
        closest_laser_reading2 = symbol_manager.get_symbol(
            self.ref_str + ".closest_laser_reading_pc"
        )
        ...
        if not self.drive_back:
            last_target_age = symbol_manager.get_symbol(
                self.ref_str + ".last_target_age"
            )
            target_lost = Monitor(name="target out of sight")
            self.add_monitor(target_lost)
            target_lost.observation_expression = cas.greater_equal(
                last_target_age, self.target_age_threshold
            )

        next_x = symbol_manager.get_symbol(
            self.ref_str + ".get_current_target()['next_x']"
        )
        next_y = symbol_manager.get_symbol(
            self.ref_str + ".get_current_target()['next_y']"
        )
        closest_x = symbol_manager.get_symbol(
            self.ref_str + ".get_current_target()['closest_x']"
        )
        closest_y = symbol_manager.get_symbol(
            self.ref_str + ".get_current_target()['closest_y']"
        )
        ...
        if self.drive_back:
            ...
            map_P_human = cas.Point3(
                (
                    symbol_manager.get_symbol(self.ref_str + ".human_point.point.x"),
        ...
        if self.drive_back:
            ...
            root_P_between_tip_and_closest = (
                root_P_closest_point + root_V_tip_to_closest / 2
            )
        ...
        map_goal_angle = cas.angle_between_vector(
            cas.Vector3([1, 0, 0]), root_V_goal_axis
        )
        map_goal_angle = cas.if_greater(
            root_V_goal_axis.y, 0, map_goal_angle, -map_goal_angle
        )
        map_angle_error = cas.shortest_angular_distance(
            map_current_angle, map_goal_angle
        )
        if self.drive_back:
            buffer = 0
        else:
        ...
        orient_to_goal.add_inequality_constraint(
        ...
        if not self.drive_back:
            look_at_target.pause_condition = (
                target_lost.get_observation_state_expression()
            )
        look_at_target.add_vector_goal_constraints(
            frame_V_current=root_V_camera_axis,
        ...
        if self.drive_back:
            ...
            oriented_towards_next.observation_expression = (
                cas.abs(map_angle_error) > self.base_orientation_threshold
            )
            self.add_monitor(oriented_towards_next)

            follow_next_point.pause_condition = (
                laser_violated.get_observation_state_expression()
            ...
            target_too_close.observation_expression = cas.less_equal(
                distance_to_human, self.min_distance_to_target
            )
        ...
        follow_next_point.add_point_goal_constraints(
            frame_P_current=root_P_bf,
        ...
        stay_in_circle.add_inequality_constraint(
            task_expression=distance_to_closest_point,
        ...
        if self.enable_laser_avoidance:
            ...
            map_V_laser_avoidance_direction = root_T_bf.dot(
            ...
            active.observation_expression = cas.greater(
                distance_to_closest_point, self.traj_tracking_radius
            )

            buffer = self.laser_avoidance_sideways_buffer / 2

            laser_avoidance_task.pause_condition = (
                active.get_observation_state_expression()
            )
            laser_avoidance_task.add_inequality_constraint(
                reference_velocity=self.max_translation_velocity,
        ...
        if self.drive_back:
            first_traj_x = symbol_manager.get_symbol(
                self.ref_str + ".get_first_traj_point()[0]"
            )
            first_traj_y = symbol_manager.get_symbol(
                self.ref_str + ".get_first_traj_point()[1]"
            )
            first_point = cas.Point3([first_traj_x, first_traj_y, 0])
            goal_reached = Monitor(name="goal reached?")
            self.add_monitor(goal_reached)
            goal_reached.observation_expression = (
    ...
    def muddle_laser_scan(self, scan: LaserScan, thresholds: np.ndarray):
        ...
        x_below_laser_avoidance_threshold1 = (
            thresholds[:, -1] > self.laser_avoidance_angle_cutout
        )
        x_below_laser_avoidance_threshold2 = (
            thresholds[:, -1] < -self.laser_avoidance_angle_cutout
        )
        x_below_laser_avoidance_threshold = (
            x_below_laser_avoidance_threshold1 | x_below_laser_avoidance_threshold2
        )
    ...
    def laser_cb(self, scan: LaserScan):
        ...
        (
            self.closest_laser_reading,
            self.closest_laser_left,
            self.closest_laser_right,
        ) = self.muddle_laser_scan(scan, self.thresholds)
    ...
    def point_cloud_laser_cb(self, scan: LaserScan):
        ...
        (
            self.closest_laser_reading_pc,
            self.closest_laser_left_pc,
            self.closest_laser_right_pc,
    ...
    def get_current_target(self) -> Dict[str, float]:
        ...
        if not self.drive_back:
            ...
            if self.last_target_age > self.target_age_exception_threshold:
                raise_to_blackboard(
                    ExecutionException(
                        f"lost target for longer than {self.target_age_exception_threshold}s"
                    )
                )
        else:
    ...
    def publish_laser_thresholds(self):
        ...
        p = Point()
        idx = np.where(self.thresholds[:, -1] < -self.laser_avoidance_angle_cutout)[0][
            -1
        ]
    ...
    def target_cb(self, point: PointStamped):
        try:
            ...
            if distance < self.interpolation_step_size * 2:
                CarryMyBullshit.traj_data[-1] = (
                    0.5 * CarryMyBullshit.traj_data[-1] + 0.5 * current_point
                )
            else:
                error_vector /= distance
                ranges = np.arange(
                    self.interpolation_step_size, distance, self.interpolation_step_size
                )
                interpolated_distance = distance / len(ranges)
                for i, dt in enumerate(ranges):
                    interpolated_point = (
                        last_point + error_vector * interpolated_distance * (i + 1)
                    )
